...
import { Row, Col, Table, Button, Divider, Badge, Breadcrumb, Tooltip } from 'antd'
import type { Dispatch } from 'umi'
import { connect, FormattedMessage, useIntl } from 'umi'
import _ from 'lodash'
import moment from 'moment'
import type { TypeListOT, TypeOT } from '../../data'
import { modalApproved } from '@/utils/utils'
import { ModalReject } from '@/components'

type Props = {
  dispatch: Dispatch
  dataTable: TypeListOT
  approvedOrReject?: boolean
  condition: any
}

const ListComment: FC<Props> = ({ dispatch, dataTable, condition, approvedOrReject }) => {
  ...
  const displayTime = (from: string, to: string) => {
    ...
    const timer =
      moment(to, 'DD/MM/YYYY HH:mm').valueOf() - moment(from, 'DD/MM/YYYY HH:mm').valueOf()
    // convert timer to minute
    const minutes = Math.floor(timer / (1000 * 60))
    // minute to hours
    const hours = Math.floor(timer / (1000 * 60 * 60))
    return `${hours} giờ ${minutes - 60 * hours} phút`
  }
  ...
  const dataSource =
    dataTable?.data?.map((item: TypeOT) => {
  ...
  const columns: any = [
    ...
    },
    {
      ...
      render: (_value: any) => (
        <Tooltip
          title={
            <p style={{ whiteSpace: 'nowrap', margin: 0 }}>{`${_value.from} -> ${_value.to}`}</p>
          }
        >
          {displayTime(_value.from, _value.to)}
        </Tooltip>
      ),
    },
    {
...
export default connect(
  ({
    ...
    dataTable: ot,
    approvedOrReject: loading.effects['ot/approvedOrReject'],
  }),
)(ListComment)
